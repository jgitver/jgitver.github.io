...
=== Manual addition of _.mvn/extensions.xml_

* Create a directory `.mvn` under the root directory of your project.
* Create file `.mvn/extensions.xml`
* Add the following content to `.mvn/extensions.xml` (adapt to http://search.maven.org/#search%7Cga%7C1%7Cg%3A%22fr.brouillard.oss%22%20a%3A%22jgitver-maven-plugin%22[latest version]).
...
Once you have created the extensions file, your project versioning is now handled by {jgitver-doc}[jgitver].
